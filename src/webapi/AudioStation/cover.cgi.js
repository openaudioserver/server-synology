...
async function coverImage (library, options) {
  ...
  if (options.id && options.id.startsWith('dir_')) {
    ...
    for (const folderPath of folderPaths) {
      ...
      if (folder.id === options.id) {
        const songs = library.media.filter(song => song.path.startsWith(folderPath))
  ...
  if (options.album_name) {
    ...
    const songs = library.media.filter(song => song.album === options.album_name && song.artist === options.album_artist_name)
    for (const song of songs) {
      if (song.hasImage) {
        const metaData = await MusicMetaData.parseFile(song.path)
  ...
  if (options.artist_name) {
    ...
    const songs = library.media.filter(song => song.artist === options.artist_name)
    for (const song of songs) {
      if (song.hasImage) {
        const metaData = await MusicMetaData.parseFile(song.path)
