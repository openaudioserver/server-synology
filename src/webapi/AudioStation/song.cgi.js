...
async function listSongs (library, options) {
  ...
  if (options.composer) {
    ...
    songResponse.data.songs = songResponse.data.songs.filter(song => song.composers.indexOf(composer.id) > -1)
  }
  if (options.genre) {
    const genre = library.genres.filter(genre => genre.name === options.genre)[0]
    songResponse.data.songs = songResponse.data.songs.filter(song => song.genres.indexOf(genre.id) > -1)
  }
  if (options.artist) {
    const artist = library.artists.filter(artist => artist.name === options.artist)[0]
    songResponse.data.songs = songResponse.data.songs.filter(song => song.artists.indexOf(artist.id) > -1)
  }
  if (options.sort_by) {
